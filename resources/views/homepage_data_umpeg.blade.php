...
<body>
    ...
    <div class="container">
        ...
        @if(@$dataumpeg && @$dataumpeg->count() > 0)
        <div class="grid-menu">
            @foreach ($dataumpeg as $s)
            <div class="list-item flex justify-between items-center p-4 mb-4 border rounded shadow bg-white">
                <h5>{{$s->navigasisekre_nama}}</h5>
                <p>{{$s->navigasisekre_deskripsi}}</p>
                @if($datasubumpeg && $datasubumpeg->count() > 0)
                @foreach ($datasubumpeg as $sub)
                <a href="{{ $sub->subnavigasisekre_link }}"
